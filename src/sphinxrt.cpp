...
#if USE_WINDOWS
#undef rename
int sph_rename ( const char * sOld, const char * sNew )
{
...
void RtIndex_t::SaveMeta ( int iDiskChunks )
{
	...
	// rename
	if ( ::sph_rename ( sMetaNew.cstr(), sMeta.cstr() ) )
...
bool RtIndex_t::SaveRamChunk ()
{
	...
	// rename
	if ( ::sph_rename ( sNewChunk.cstr(), sChunk.cstr() ) )
...
void RtBinlog_c::SaveMeta ()
{
	...
	if ( ::sph_rename ( sMeta.cstr(), sMetaOld.cstr() ) )
		sphDie ( "failed to rename meta (src=%s, dst=%s, errno=%d, error=%s)",
